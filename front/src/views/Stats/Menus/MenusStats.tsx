...
export default function MenusStats() {
  ...
  const [tabMenuStats, setTabMenuStats] = useState<TabMenuStats>({
    status: "day",
  }); // 일, 주, 월에 따른 탭 메뉴 선택
        ...
        <div css={css.menuStatsLeft}>
          <div css={css.menuStatsLeftTopContainer}>
            <div css={css.menuStatsLeftTopLeft}>
              ...
              <div css={css.yesterday}>주</div>
              <div css={css.thisMonth}>달</div>
            </div>
            <div
              css={css.calendarContainer}
              onClick={() => {
            ...
            >
              <div>
                <EventAvailableIcon
                  sx={{ fontSize: 26, margin: "10px 5px 0  5px" }}
                />
              </div>
          ...
          <div css={css.menuStatsRightTextContainer}>
            <div css={css.orderProductName}>
              ...
              {data.map((item: any, index: number) => (
                <div key={index}>{item.orderProductName}</div>
              ))}
            </div>
            ...
            <div css={css.totalQuantitySold}>
              <div>판매 개수</div>
              {data.map((item: any, index: number) => (
                <div key={index}>{item.totalQuantitySold}</div>
              ))}
            </div>
            ...
            <div css={css.totalPrice}>
              <div>총 판매 가격</div>
              {data.map((item: any, index: number) => (
                <div key={index}>{item.totalPrice}</div>
              ))}
            </div>
          </div>
        </div>
